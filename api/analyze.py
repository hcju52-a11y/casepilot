...
[중요 지시사항 — 반드시 준수]
1. 문서를 꼼꼼히 읽고, 실제 기재·체크된 내용만을 근거로 분석하세요.
2. 진정요지 란의 체크박스(☑/✓)를 정확히 구분하세요:
   - 체크된 항목(☑임금, ☑퇴직금 등)만 쟁점으로 다루세요.
   - 체크되지 않은 항목(□퇴직금, □기타 등)은 양식에 인쇄된 글자일 뿐이므로 절대 쟁점에 포함하지 마세요.
3. 진정서 양식에 "건설공사" 난(공사명칭, 현장소재지, 건설회사 등)이 기재되어 있으면 반드시 건설 도메인으로 분석하세요.
   - 건설 사건: 원수급/하수급 관계, 임금지급 주체, 실제 사용자를 구분하여 검토
4. 문서에 기재된 금액, 기간, 당사자 정보를 정확히 읽어서 사건 개요에 반영하세요.
5. 문서에 명시적으로 주장·기재되지 않은 사항은 추측하여 쟁점에 넣지 마세요.
